...
import { getTournamentById, getTournamentsByOrgId } from "@/lib/actions/tournament.actions";
...
export default function OrganizationPageResume() {
  ...
  const fetchTournamentsByOrg = async (orgId: string) => {
    try {
      const data = await getTournamentsByOrgId(orgId as string);
      setTournaments(data ?? null);
  ...
  useEffect(() => {
    if (activeOrg?.id) {
      fetchTournamentsByOrg(activeOrg.id);
    }
  }, [activeOrg?.id]);
      ...
      {!statsLoading ? (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 28 }}>
            {[
              { label: "Tournois actifs", value: tournaments?.length, delta: "+1", color: "var(--accent)", icon: Icons.trophy },
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--muted)" }}>
          <p style={{ fontFamily: "Syne, sans-serif", fontWeight: 600 }}>Chargement des statistiques...</p>
        </div>
      )}
    </>
  );
